...
class Label(object):
    ...
    tape_size: Tuple[int, int] = attrib()
    #: The type of label
    form_factor: FormFactor = attrib()
    #: The total area (width, length) of the label in dots (@300dpi).
    dots_total: Tuple[int, int] = attrib()
    #: The printable area (width, length) of the label in dots (@300dpi).
    dots_printable: Tuple[int, int] = attrib()
    ...
    feed_margin: int = attrib(default=0)
    ...
    restricted_to_models: List[str] = attrib(factory=list)
